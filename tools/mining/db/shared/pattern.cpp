...
bool inv_pair_boundary(const Pattern& p) {
  for (auto* c : p.cells) {
    if (!is_inv(c)) continue;
    for (auto& [pn, nets] : c->fanouts) {
      for (auto* n : nets) {
        for (auto* l : n->loads) {
          auto* nxt = l ? l->cell : nullptr;
          if (!nxt || p.cells.count(nxt) == 0 || !is_inv(nxt)) continue;
          bool up = false;
          for (auto& [ip, in] : c->fanins) {
            auto* drv = in ? in->driver : nullptr;
            auto* prev = drv ? drv->cell : nullptr;
            if (prev && prev != nxt && p.cells.count(prev)) { up = true; break; }
          }
          bool down = false;
          for (auto& [op, ons] : nxt->fanouts) {
            for (auto* on : ons) {
              for (auto* l2 : on->loads) {
                auto* succ = l2 ? l2->cell : nullptr;
                if (succ && succ != c && p.cells.count(succ)) { down = true; break; }
              }
              if (down) break;
            }
            if (down) break;
...
int boundary_output_count(const Pattern& p) {
  int count = 0;
