...
struct CellMetricResult {
  ...
  int cpp_pmos = 0;
  int cpp_min  = 0;
  int euler_breaks_nmos = 0;
  int euler_breaks_pmos = 0;
  int cpp_min_tight = 0;
...
inline CellData parse_subckt(const std::string& text, const std::string& cell_name) {
  ...
  while (std::getline(iss, line)) {
    ...
    if (!current.empty()) {
      ...
      if (!in_target) {
        ...
        if (up == ".ENDS" || up.rfind(".ENDS ", 0) == 0) { saw_ends = true; break; }
        auto toks = cm_split_ws(stmt);
        if (toks.empty()) continue;
        const char lead = static_cast<char>(std::toupper(static_cast<unsigned char>(toks[0][0])));

        if (lead != 'M') {
          throw std::runtime_error("cell_metrics: unsupported non-MOS element '" + toks[0] +
              "' inside subckt '" + cell.name + "': " + stmt +
              " (only M<name> transistor lines are supported; a hierarchical or "
              "RC-annotated netlist cannot be analyzed flat)");
        }
  ...
  if (!current.empty() && in_target && !saw_ends) {
    ...
    if (up == ".ENDS" || up.rfind(".ENDS ", 0) == 0) {
      ...
      if (!toks.empty() && std::toupper(static_cast<unsigned char>(toks[0][0])) != 'M') {
        throw std::runtime_error("cell_metrics: unsupported non-MOS element '" + toks[0] +
            "' inside subckt '" + cell.name + "' (EOF line): " + current +
            " (only M<name> transistor lines are supported)");
      }
      if (!toks.empty()) {
        Device d;
        ...
        if (!d.is_pmos && !d.is_nmos)
          throw std::runtime_error("cell_metrics: unknown MOS model type in subckt '" +
              cell.name + "' (EOF line): '" + d.model + "'");
        if (d.width <= 0.0 || d.length <= 0.0)
          throw std::runtime_error("cell_metrics: device '" + d.name + "' in subckt '" +
              cell.name + "' (EOF line) missing width or length");
        cell.devices.push_back(d);
      }
    }
  }
  ...
  if (in_target && !saw_ends) {
    throw std::runtime_error("cell_metrics: subckt '" + cell_name +
...
inline std::unordered_map<std::string, CellData> parse_all_subckts(const std::string& text) {
  ...
  auto process_stmt = [&](const std::string& stmt) {
    ...
    if (!in_subckt) {
      ...
      if (up == ".ENDS" || up.rfind(".ENDS ", 0) == 0) {
        if (!cell.name.empty()) {
          auto [it, inserted] = result.emplace(cell.name, std::move(cell));
          if (!inserted) {
            throw std::runtime_error(
                "duplicate .SUBCKT definition for '" + it->first +
                "' in CDL. Clean up the source file: the first definition is kept, "
                "duplicates silently overwriting pin names would corrupt scoring.");
          }
        }
        cell = CellData{};
        in_subckt = false;
      ...
      if (toks.size() < 6)
        throw std::runtime_error("cell_metrics: malformed MOS line in subckt '" +
            cell.name + "' (need >=6 tokens): " + stmt);
      Device d;
      d.name = toks[0];
      d.drain = toks[1];
      d.gate = toks[2];
      ...
      if (!d.is_pmos && !d.is_nmos)
        throw std::runtime_error("cell_metrics: unknown MOS model type in subckt '" +
            cell.name + "': '" + d.model + "' (expected substring 'pmos' or 'nmos')");
      bool has_nfin = false;
      for (size_t i = 6; i < toks.size(); ++i) {
        const std::string tok = toks[i];
        const auto eq = tok.find('=');
        ...
        try {
          ...
          throw std::runtime_error("cell_metrics: failed to parse device parameter '" +
              key + "=" + val + "': " + e.what());
        }
      }
      if (d.width <= 0.0 || d.length <= 0.0)
        throw std::runtime_error("cell_metrics: device '" + d.name + "' in subckt '" +
            cell.name + "' missing width or length (w=" + std::to_string(d.width) +
            " l=" + std::to_string(d.length) + "); check 'w=' and 'l=' params in CDL");
  ...
  if (in_subckt) {
    throw std::runtime_error("cell_metrics: subckt '" + cell.name +
        "' reached end of file without a closing .ENDS (truncated CDL)");
  }
  return result;
}
...
inline void enumerate_paths_dfs_metrics(
  ...
  for (const auto& d : cell.devices) {
    ...
    enumerate_paths_dfs_metrics(
        ...
        resistance_proxy + ((d.width > 0.0) ? (d.length / d.width) : 0.0),
        metrics);
  ...
  for (const auto& node : candidates) {
    ...
    for (const auto& d : cell.devices) {
      if (d.drain == node || d.source == node) {
        m.local_capacitance_proxy += diff_cap_ratio * d.width * d.length;
      }
      if (d.gate == node) {
        m.local_capacitance_proxy += d.width * d.length;
      }
    }
...
inline int count_euler_breaks(const std::vector<Device>& devices, bool pmos) {
  std::unordered_map<std::string, int> degree;
  for (const auto& d : devices) {
    if (d.is_pmos != pmos) continue;
    degree[d.drain]++;
    degree[d.source]++;
  }
  int odd_count = 0;
  ...
  return (odd_count > 2) ? (odd_count - 2) / 2 : 0;
}
...
inline CellMetricResult compute_cell_metrics(const CellData& cell, double diff_cap_ratio = 1.0,
  ...
  result.euler_breaks_nmos = count_euler_breaks(cell.devices, false);
  result.euler_breaks_pmos = count_euler_breaks(cell.devices, true);
  result.cpp_min_tight = std::max(result.cpp_nmos + result.euler_breaks_nmos,
                                   result.cpp_pmos + result.euler_breaks_pmos);
  ...
  for (const auto& op : result.pins.output_pins) {
    ...
    const auto capacitance_range = sensitized_output_proxy_range(
        op, cell, result.pins, stage_metrics,
        [](const StageMetric&, bool, const Device& d) { return d.width * d.length; });
