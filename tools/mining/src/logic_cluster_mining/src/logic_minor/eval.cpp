...
class Expr {
 ...
 private:
  ...
  bool parse_unary() {
    ...
    if (t_ != END) {
      std::cerr << "[warning] eval: unexpected token '" << tok_
                << "' in expr: " << s_ << "\n";
    }
    return false;
  }
...
PatternEvalSummary evaluate_pattern_outputs(const Pattern& p, bool ignore_nonroot_outputs, bool reduce_arity) {
  ...
  if (!ignore_nonroot_outputs) for (auto* c : p.cells) {
  ...
  if (boundary_outputs.size() == 1) {
    ...
    std::string tt_bits = output_bits[0];
    int tt_n = n;

    if (reduce_arity && tt_n > 1) {

      int rows = 1 << tt_n;
      std::vector<bool> dep(tt_n, false);
      for (int vi = 0; vi < tt_n; ++vi) {
        int flip = 1 << (tt_n - 1 - vi);
        for (int idx = 0; idx < rows; ++idx) {
          if (tt_bits[idx] != tt_bits[idx ^ flip]) { dep[vi] = true; break; }
        }
      }
      int new_n = 0;
      for (bool d : dep) if (d) new_n++;
      if (new_n < tt_n) {
        std::string new_bits;
        new_bits.reserve(static_cast<size_t>(1 << new_n));
        for (int idx = 0; idx < rows; ++idx) {
          bool keep = true;
          for (int vi = 0; vi < tt_n; ++vi) {
            if (!dep[vi] && ((idx >> (tt_n - 1 - vi)) & 1)) { keep = false; break; }
          }
          if (keep) new_bits.push_back(tt_bits[idx]);
        }

        std::vector<std::string> reduced_order;
        reduced_order.reserve(static_cast<size_t>(new_n));
        for (int vi = 0; vi < tt_n; ++vi) {
          if (dep[vi]) reduced_order.push_back(keys[static_cast<size_t>(vi)]);
        }
        out.input_net_order = std::move(reduced_order);
        tt_bits = std::move(new_bits);
        tt_n = new_n;
      }
    }

    if (out.input_net_order.size() != static_cast<size_t>(tt_n) ||
        tt_bits.size() != static_cast<size_t>(1) << tt_n) {
      throw std::runtime_error("truth table / input order mismatch: " + std::to_string(tt_bits.size()) +
                               " rows, arity " + std::to_string(tt_n) + ", " +
                               std::to_string(out.input_net_order.size()) + " input names");
    }

    out.vec_key = std::to_string(tt_n) + "|" + tt_bits;
    out.canonical_key = vec_to_canon(out.vec_key);
    std::unordered_set<std::string> canonicals;
    canonicals.insert(out.canonical_key);
    out.signature = canonical_set_signature(canonicals);
    out.raw_truth_table = tt_bits;
  } else {
