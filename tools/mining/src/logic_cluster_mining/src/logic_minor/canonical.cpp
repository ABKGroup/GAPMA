...
class Expr {
 ...
 private:
  ...
  bool parse_unary() {
    ...
    if (t_ != END) {
      std::cerr << "[warning] canonical eval: unexpected token '" << tok_
                << "' in expr: " << s_ << "\n";
    }
    return false;
  }
...
bool complement_is_positive_unate(const std::string& bits, int n) {
  const int rows = 1 << n;
  for (int var = 0; var < n; ++var) {
    const int bit = 1 << (n - 1 - var);
    for (int idx0 = 0; idx0 < rows; ++idx0) {
      if (idx0 & bit) continue;
      const int idx1 = idx0 | bit;
      const char h0 = (bits[idx0] == '0') ? '1' : '0';
      const char h1 = (bits[idx1] == '0') ? '1' : '0';
...
std::vector<NpnCanonical> npn_canonical_infos(const std::string& bits, int n) {
  ...
  int best_cost = 0;
  std::unordered_map<std::string, NpnCanonical> best;
  for (int neg_mask = 0; neg_mask < rows; ++neg_mask) {
    const int input_cost = __builtin_popcount(static_cast<unsigned>(neg_mask));
    for (int output_flip = 0; output_flip <= 1; ++output_flip) {
      std::string nb(rows, '0');
      for (int i = 0; i < rows; ++i) {
        char v = bits[i ^ neg_mask];
        if (output_flip) v = (v == '0') ? '1' : '0';
        nb[i] = v;
      }
      std::string cand = canonical_bits(nb, n);
      if (!complement_is_positive_unate(cand, n)) continue;
      const int cost = input_cost + output_flip;
      ...
      if (cost == best_cost) {
        auto& out = best[cand];
        out.bits = cand;
        out.erased_input_inverters = input_cost;
        out.erased_output_inverters = output_flip;
        out.erased_total_inverters = cost;
      }
    }
  }

  std::vector<NpnCanonical> outs;
  outs.reserve(best.size());
  for (auto& kv : best) outs.push_back(std::move(kv.second));
  std::sort(outs.begin(), outs.end(), [](const NpnCanonical& a, const NpnCanonical& b) {
    return a.bits < b.bits;
  });
  if (outs.empty()) {
    bool have_fallback = false;
    int best_cost = 0;
    std::unordered_map<std::string, NpnCanonical> fallback;
    for (int neg_mask = 0; neg_mask < rows; ++neg_mask) {
      const int input_cost = __builtin_popcount(static_cast<unsigned>(neg_mask));
      for (int output_flip = 0; output_flip <= 1; ++output_flip) {
        std::string nb(rows, '0');
        ...
        std::string cand = canonical_bits(nb, n);
        if (cand.empty() || cand[0] != '1') continue;
        const int cost = input_cost + output_flip;
        if (!have_fallback || cost < best_cost) {
          have_fallback = true;
          best_cost = cost;
          fallback.clear();
        }
        if (cost == best_cost) {
          auto& out = fallback[cand];
          out.bits = cand;
          out.erased_input_inverters = input_cost;
          out.erased_output_inverters = output_flip;
          out.erased_total_inverters = cost;
    ...
    outs.reserve(fallback.size());
    for (auto& kv : fallback) outs.push_back(std::move(kv.second));
    std::sort(outs.begin(), outs.end(), [](const NpnCanonical& a, const NpnCanonical& b) {
      return a.bits < b.bits;
    });
  }
...
std::string key_to_binary(const std::string& k) {
  ...
  if (p == std::string::npos)
    throw std::runtime_error("key_to_binary: malformed canonical key (no '#0x'): '" + k + "'");
...
std::string expr_from_canon(const std::string& k) {
  ...
  if (b.rfind("0b", 0) != 0)
    throw std::runtime_error("expr_from_canon: key_to_binary returned non-binary for key: '" + k + "'");
