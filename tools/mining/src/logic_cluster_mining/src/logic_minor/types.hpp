...
struct Opt {
  ...
  bool ignore_nonroot_outputs = false;
...
struct Cfg {
  ...
  fs::path freq_file;
  fs::path detail_file;
  fs::path overlap_file;
  fs::path subset_file;
  fs::path map_file;
  fs::path cov_file;
  fs::path graph_file;
  fs::path config_log_file;
  fs::path runtime_log_file;
  fs::path included_clusters_file;
  fs::path excluded_clusters_file;
  fs::path branch_log_file;
  fs::path verification_report_file;
  fs::path selected_candidates_file;
  fs::path np_class_map_file;
  fs::path cdb_file;
  fs::path cell_db_input;
  fs::path netlist_db_input;
  bool text_outputs = true;
  bool verification_mode = false;
  bool no_structural_hash = false;
  bool add_np_class = true;
};
